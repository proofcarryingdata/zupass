...
describe("ZKEdDSAFrogNoirPCD should work", function () {
  ...
  async function toArgs(frogData: IFrogData): Promise<ZKEdDSAFrogNoirPCDArgs> {
    ...
    const ret: ZKEdDSAFrogNoirPCDArgs = {
      frog: {
  ...
  it("should be able to generate and verify a valid proof", async function () {
    ...
    const verificationRes = await ZKEdDSAFrogNoirPCDPackage.verify(pcd);
  ...
  async function testProveBadArgs(
    validArgs: ZKEdDSAFrogNoirPCDArgs,
    mutateArgs: (args: ZKEdDSAFrogNoirPCDArgs) => Promise<void>
  ): Promise<void> {
    // Clone validArgs to mutate them into invalidArgs while keeping originals.
    const invalidArgs: ZKEdDSAFrogNoirPCDArgs = JSON.parse(
      JSON.stringify(validArgs)
    );
    await mutateArgs(invalidArgs);
    await assert.rejects(async () => {
      await ZKEdDSAFrogNoirPCDPackage.prove(invalidArgs);
    });
  }
  ...
  async function testProveBadFrogArgs(
    ...
    await testProveBadArgs(validArgs, async (args: ZKEdDSAFrogNoirPCDArgs) => {
  ...
  it("should not verify a proof with incorrect frog claims", async function () {
    const pcdArgs = await toArgs(frogData);
    const validPCD = await ZKEdDSAFrogNoirPCDPackage.prove(pcdArgs);

    await testVerifyBadClaim(validPCD, (claim: ZKEdDSAFrogNoirPCDClaim) => {
  ...
  it("should be able to serialize and deserialize a PCD", async function () {
    ...
    const deserializedValid =
      await ZKEdDSAFrogNoirPCDPackage.verify(deserialized);
