 ...
 */
  public static generateSignals(listIndices: number[],
				membershipLists: bigint[][],
  ...
  if (listIndices.length != membershipLists.length)
    throw new TypeError("The lengths of the index and value list arrays must be the same.");
  if (membershipLists.reduce((prod, lis) => prod*lis.length, 0) > paramMaxListEntries)
    throw new TypeError(`The total number of entries in the list membership check exceeds MAX_LIST_ENTRIES (${paramMaxListEntries}).`);
  if ((paramTupleArity < 2) || (paramTupleArity > 4))
    throw new TypeError("The arity must lie between 2 and 4.");
  listIndices.forEach(i => {if(i >= paramMaxValues) throw new Error("The indices cannot exceed MAX_VALUES.");});

  // Helpers
  const pad = (arr: any, len: number, padding: any) => arr.concat(Array(len - arr.length).fill(padding));
  
  // Sort input for permutation invariance and hash the individual membership lists.
  const sortedZippedList: [number, bigint[]][] = listIndices
    .map(function(i, j): [number, bigint[]] { return [i, membershipLists[j]]; })
    .sort((a, b) => a[0] - b[0]) // Sort indices
    .map(pair => [pair[0], pair[1].sort().map(x => poseidon1([x]))]); // Sort membership lists and hash the elements

  // Early return if tuples are not required.
  if(listIndices.length == 1)
    return [listIndices[0], [[]], pad(sortedZippedList[0][1], paramMaxListEntries, sortedZippedList[0][1][0])];

  // Form tupleIndices
  const unpaddedTupleIndices: bigint[][] =
    (sortedIndices => {
      const unpaddedTupleIndices = sortedIndices // Take the sorted indices
        .reduce((tupleIndices: number[][], currentIndex: number) => { // and inspect the current index as well as the indices grouped so far.
	  const l = tupleIndices.length;
	  return (tupleIndices[l - 1].length < paramTupleArity) // If the last index group is not at arity,
	    ? tupleIndices.slice(0, l-1).concat([tupleIndices[l-1].concat([currentIndex])]) // append the current index to it.
	...
	? unpaddedTupleIndices.slice(0, unpaddedTupleIndices.length-1).concat([pad(lastIndices, paramTupleArity, BABY_JUB_NEGATIVE_ONE)]) // fill up.
	: unpaddedTupleIndices;
    })(sortedZippedList.map((x: [number, bigint[]]) => x[0]));
  
  if (unpaddedTupleIndices.length > paramTupleArity)
    throw new TypeError(`The number of tuple indices (${unpaddedTupleIndices.length}) exceeds MAX_TUPLES (${paramMaxTuples}).`);

  const memberIndex = paramMaxValues + unpaddedTupleIndices.length - 1;
  
  const tupleIndices = pad(unpaddedTupleIndices, paramMaxTuples, pad([], paramTupleArity, BABY_JUB_NEGATIVE_ONE));

  // Choose appropriate Poseidon hash and form hash reducer.
  const hash = poseidon[paramTupleArity-2];
  const hashReduce = ((lis: bigint[]) => // Take the list
    ((reduction: bigint[]) => (reduction.length == 1) ? reduction[0] : hash(pad(reduction, paramTupleArity, 0))) // After performing the reduction, make sure the reduction led to a single value by padding with 0s,
		      (((l: bigint[]) => l.reduce( (acc: bigint[], x: bigint) => (acc.length < paramTupleArity) ? acc.concat([x]) : [hash(acc), x], []))(lis))); // where the reduction consists of grouping elements of the list in lists of size `paramTupleArity`.  
  ...
  const membershipListsToTupleList: Fn = (listList: bigint[][], acc = []) =>
    (listList.length == 0) ? [hashReduce(acc)]
    : listList[0].flatMap((x: bigint) => membershipListsToTupleList(listList.slice(1), acc.concat([x])));
  
  // Pad membership list if necessary.
  const tupleMembershipList: bigint[] = (unpaddedArray => pad(unpaddedArray, paramMaxListEntries, unpaddedArray[0]))
