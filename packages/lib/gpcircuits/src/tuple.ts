...
export class Tuple {
  ...
  public static tupleHashes(
    ...
    if (valueHash.length != paramMaxValues)
      throw new TypeError(
        `The length of valueHash (${valueHash.length}) is not equal to ${paramMaxValues}.`
      );

    // Check the dimensions of `tupleIndices`.
    if (tupleIndices.length != paramMaxTuples)
      throw new TypeError(
        `The number of tuple indices given (${tupleIndices.length}) should equal ${paramMaxTuples}.`
      );
    tupleIndices.forEach((nthTupleIndices, i) => {
      if (nthTupleIndices.length != paramTupleArity)
        throw new TypeError(
          `The arity of tupleIndices[${i}] is not equal to the fixed tuple arity (${paramTupleArity}).`
        );
    });
    ...
    const tupleHashes = tupleIndices.reduce(
      (tupleHashes, indexArray) =>
        tupleHashes.concat([
          hash(
            indexArray.map((i) =>
              i < BigInt(paramMaxValues)
                ? valueHash[Number(i)] // Conversion OK since `i` is small.
                : i < BigInt(paramMaxValues + tupleHashes.length)
                  ? tupleHashes[Number(i) - paramMaxValues] // `i` is still small.
                  : 0
            )
          )
        ]),
