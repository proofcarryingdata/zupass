...
async function main() {
  ...
  const iconIndex = await Promise.all(iconPaths.map(async (iconPath) => {
    return [
      path.basename(iconPath, ".svg"),
      await svgToUrl(iconPath)
    ]
  }));

  const output = iconIndex.map(([name, dataUrl]) => {
