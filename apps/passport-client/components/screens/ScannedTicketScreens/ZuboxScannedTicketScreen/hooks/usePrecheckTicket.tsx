...
import {
  ZuboxActionPreCheckResult,
  createTicketActionCredentialPayload,
  requestZuboxPreCheck
 ...
 * request to the Zubox backend to determine what it can do with a ticket.
...
export function usePreCheckTicket(
  ...
  | {
      loading: false;
      result: ZuboxActionPreCheckResult;
    } {
  const [inProgress, setInProgress] = useState(true);
  const [result, setResult] = useState<ZuboxActionPreCheckResult | undefined>();
  ...
  const doPreCheckTicket = useCallback(
    async (ticketId: string | undefined, eventId: string | undefined) => {
      ...
      const preCheckTicketResult = await requestZuboxPreCheck(
