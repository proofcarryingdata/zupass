...
export function PodboxScannedTicketScreen(): JSX.Element {
  ...
  useEffect(() => {
    if (self == null || userForcedToLogout) {
      ...
      setPendingPodboxScannedRequest(stringifiedRequest);
      if (self == null) {
        window.location.href = `/#/login?redirectedFromAction=true&${pendingPodboxScannedRequestKey}=${encodeURIComponent(
          stringifiedRequest
        )}`;
      }
    }
